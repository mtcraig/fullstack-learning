...
app.get('/random', (req,res) => {
  const rI = Math.floor((Math.random() * jokes.length));
  res.json(jokes[rI]);
})
...
app.get('/filter', (req,res) => {
  ...
  const foundJokes = jokes.filter((joke) => joke.jokeType === changeCase.capitalCase(jokeType));
  res.json(foundJokes);
})
...
app.patch('/jokes/:id', (req,res) => {
  ...
  const updJoke = {
    ...
    jokeType: req.body.type ? req.body.type : origJoke.jokeType
  };
  console.log(updJoke);
  const searchIndex = jokes.findIndex((joke) => joke.id === id);
  jokes[searchIndex] = updJoke;
...
app.delete('/jokes/:id', (req,res) => {
  ...
  if (searchIndex > -1) {
    ...
    res.status(404).json({error: `Joke with id: ${id} was not found.`});
  }
})
